---
title: "A basic introduction to runoutSIM"
author: "Jason Goetz"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{A basic introduction to runoutSIM}
...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  screenshot.force = TRUE,   # Force static screenshots for widgets
  fig.retina = 1,            # Keep PNG image file sizes small
...
The `runoutSIM` package is developed for spatially simulating landslide runout across large areas using random walks. This document provides an introduction to applying `runoutSIM` for regional debris flow simulation using a subcatchment of the upper Maipo river basin, Chile as an example. In this vignette, we will:
...
Spatial handling in the `runoutSIM` package is supported by the `sf` (fr vector data) and `terra` (for gridded/raster data) packages. In this example, we load/read a digital elevation model (DEM) with 12.5 m spatial resolution, polygons showing where debris flows have occurred, and corresponding source points mapped for each polygon. We also create a hillshade model to help with visualization of the results.
...
# Simulate runout from a single source cell
...
-   Traverse frequencies for each grid cell, which shows the most likely flow paths

-   Maximum velocities (m/s) for each grid cell

These values are stored as cell numbers. `prob_connect` is a value that can be calculated separately that provides the probability debris from the source cell can reach another feature of interest.
...
We use `velocityToRaster()` to create a raster of the velocity values of the debris flow (in m/s).
...
The `runoutSim()` function also allows us to calculate the propotion of simulated debris paths that intersect a feature from a given source cell (i.e. connectivity probability).
...
Now that we have a connectivity feature, we can re-run `runoutSim()` with some additional parameter options: `source_connect = TRUE` allowing us to calculate a connectivity probability and `connect_feature = feature_mask`.
...
```{r, ffig.show='hold', fig.width=8, fig.height=5}
