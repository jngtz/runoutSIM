...
runoutSim_rockfall <- function(dem, xy, mu = 0.1, md = 40, int_vel = 1, slp_thresh = 30, exp_div = 3, per_fct = 2, walks = 100,
  ...
  for(k in 1:walks){
    ...
    while(n == 0){
      ...
      # get elevation of neighboring cells lower than local cell
      elv_ngh <- elv_ngh[lower_elv]
      ...
      v_i <- rockfall(mu = mu_in, v_p = v_p, h_i = elv_ngh[nxt_cell == cells], h_p = elv_cntr, 
                      K=0.25, method = 'rolling', l = cell_dist[nxt_cell])
